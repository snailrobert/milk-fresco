...
public class BViewHolder extends BaseViewHolder<BBean> {
    ...
    @Override
    public void bindView(final List<BBean> beans) {
        final BBean bean = beans.get(getAdapterPosition());
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<BBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (BBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_ISAOB,
                    "下载地址：" + bean.getUrl()));
        }
        return imgs;
    }
