...
public class YHViewHolder extends BaseViewHolder<YHBean> {
    ...
    public YHViewHolder(View itemView) {
        super(itemView);
    }

    @Override
    public void bindView(final List<YHBean> beans) {
        final YHBean bean = beans.get(getAdapterPosition());
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<YHBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (YHBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_MEINVYH,
