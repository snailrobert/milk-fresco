...
public class MenViewHolder extends BaseViewHolder<MenBean> {
    ...
    @Override
    public void bindView(final List<MenBean> beans) {
        final MenBean bean = beans.get(getAdapterPosition());
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<MenBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (MenBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_MEITUMEN,
                    "下载地址：" + bean.getUrl()));
        }
        return imgs;
    }
