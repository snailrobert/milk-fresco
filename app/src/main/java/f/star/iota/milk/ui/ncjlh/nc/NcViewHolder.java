...
public class NcViewHolder extends BaseViewHolder<NcBean> {
    ...
    @Override
    public void bindView(final List<NcBean> beans) {
        final NcBean bean = beans.get(getAdapterPosition());
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<NcBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (NcBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_NCJLH,
                    "下载地址：" + bean.getUrl()));
        }
        return imgs;
    }
