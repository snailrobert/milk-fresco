...
public class BaViewHolder extends BaseViewHolder<BaBean> {
    ...
    public BaViewHolder(View itemView) {
        super(itemView);
    }

    @Override
    public void bindView(final List<BaBean> beans) {
        final BaBean bean = beans.get(getAdapterPosition());
        ...
        FrescoLoader.load(mSimpleDraweeView, bean.getUrl(),bean.getHeaders(),width,height);
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<BaBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (BaBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_YULEBA,
