...
public class ZUViewHolder extends BaseViewHolder<ZUBean> {
    ...
    public ZUViewHolder(View itemView) {
        super(itemView);
    }

    @Override
    public void bindView(final List<ZUBean> beans) {
        final ZUBean bean = beans.get(getAdapterPosition());
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<ZUBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (ZUBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_80ZU,
                    "下载地址：" + bean.getUrl()));
        }
        return imgs;
    }
