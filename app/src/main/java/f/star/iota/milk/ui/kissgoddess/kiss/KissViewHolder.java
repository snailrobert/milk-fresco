...
public class KissViewHolder extends BaseViewHolder<KissBean> {
    ...
    public KissViewHolder(View itemView) {
        super(itemView);
    }

    @Override
    public void bindView(final List<KissBean> beans) {
        final KissBean bean = beans.get(getAdapterPosition());
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<KissBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (KissBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_GODDESS,
                    "下载地址：" + bean.getUrl()));
        }
        return imgs;
    }
