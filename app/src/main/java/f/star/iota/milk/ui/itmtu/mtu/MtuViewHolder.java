...
public class MtuViewHolder extends BaseViewHolder<MtuBean> {
    ...
    public MtuViewHolder(View itemView) {
        super(itemView);
    }

    @Override
    public void bindView(final List<MtuBean> beans) {
        final MtuBean bean = beans.get(getAdapterPosition());
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<MtuBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (MtuBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_ITMTU,
                    "下载地址：" + bean.getUrl()));
        }
        return imgs;
    }
