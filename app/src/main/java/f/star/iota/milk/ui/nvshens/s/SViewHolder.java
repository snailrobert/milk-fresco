...
public class SViewHolder extends BaseViewHolder<SBean> {
    ...
    @Override
    public void bindView(final List<SBean> beans) {
        final SBean bean = beans.get(getAdapterPosition());
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<SBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (SBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_NVSHENS,
                    "下载地址：" + bean.getUrl()));
        }
        return imgs;
    }
