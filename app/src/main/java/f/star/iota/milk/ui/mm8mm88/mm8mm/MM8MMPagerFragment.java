...
public class MM8MMPagerFragment extends PagerFragment {
    ...
    protected TitlePagerAdapter getPagerAdapter() {
        ...
        String s = ReadResourceUtil.readResource(R.raw.mm8);
        Elements selects = Jsoup.parseBodyFragment(s).select("ul >li");
        for(Element li : selects){
            String url = Net.MM8MM88_BASE + li.select("a").attr("href");
            String tagName = li.select("a").text();
            titles.add(tagName);
            fragments.add(MM8MMFragment.newInstance(url));
